...
// import React, { useState, useEffect } from "react";
// import { FaPlus, FaTimes, FaEdit, FaTrash, FaLink, FaSearch } from "react-icons/fa";
// import { motion } from "framer-motion";
// import axios from "axios";
...
//     const [showForm, setShowForm] = useState(false);
//     const [tasks, setTasks] = useState([]);
//     const [userStatuses, setUserStatuses] = useState({}); // Store user-specific statuses
//     const [taskData, setTaskData] = useState({
//         id: null,
//         title: "",
//         description: "",
...
//             const statusResponse = await axios.get(`http://localhost:5000/userTaskStatus?userId=${loggedInUserId}`);
//             const statuses = statusResponse.data.reduce((acc, status) => {
...
//     const handleStatusChange = async (taskId, newStatus) => {
//         try {
//             const payload = { userId: loggedInUserId, status: newStatus };
//             const response = await axios.put(`http://localhost:5000/tasks/${taskId}`, payload);

//             // Update tasks with new counts
//             const tasksResponse = await axios.get("http://localhost:5000/tasks");
//             setTasks(tasksResponse.data);

//             // Update user-specific status
//             setUserStatuses((prev) => ({ ...prev, [taskId]: newStatus }));
//         } catch (error) {
//             console.error("Error updating task status:", error);
//             alert("Failed to update task status. Please try again.");
//         }
...
//     const filterTasks = () => {
//         return tasks.filter((task) => {
//             const userStatus = userStatuses[task._id] || "To-Do"; // Default to "To-Do" if no status
//             const matchesFilter =
//                 filter === "All" ||
//                 (filter === "My Tasks" && task.userId === loggedInUserId) ||
//                 (filter === "Tasks with Attachments" && task.fileUrl) ||
//                 (filter === "Due Today" && new Date(task.dueDate).toDateString() === new Date().toDateString()) ||
//                 (filter === "Due This Week" && new Date(task.dueDate) <= new Date(new Date().setDate(new Date().getDate() + 7))) ||
//                 (filter === "Completed Tasks" && userStatus === "Completed") ||
//                 (filter === "In Progress" && userStatus === "In Progress") ||
//                 (filter === "To-Do" && userStatus === "To-Do");
...
//                             userStatus={userStatuses[task._id] || "To-Do"}
...
// const TaskCard = ({ task, loggedInUserId, userStatus, onDelete, onEdit, onStatusChange }) => {
//     const statusColors = {
//         "To-Do": "bg-gray-400",
//         "In Progress": "bg-yellow-400",
//         "Completed": "bg-green-500",
...
//             animate={{ opacity: 1, y: 0 }}
//             className="p-5 bg-white rounded-lg shadow-md border-l-4 transition-all hover:shadow-xl"
//             style={{ borderColor: statusColors[userStatus] || "gray" }}
//         >
//             <div className="flex justify-between items-center">
//                 <h4 className="font-semibold text-lg">{task.title}</h4>
//                 <span className={`text-sm font-bold px-3 py-1 rounded-full ${statusColors[userStatus]} text-white`}>
//                     {userStatus}
//                 </span>
//             </div>
//             <p className="text-gray-600 mt-2">{task.description}</p>

...
const Task = ({ loggedInUserId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (taskData.id) {
      ...
      setTasks((prevTasks) =>
        prevTasks.map((task) =>
          task.id === taskData.id ? { ...task, ...newTask } : task
        )
      );
    } else {
      // Create a new task
            ...
            <div className="flex justify-center space-x-2 text-2xl">
              <FiThumbsUp
                ...
                className={`hover:text-gray-500 duration-200 ${
                  activeReaction === "like" ? "text-blue-500" : ""
                }`}
              />
              <FiThumbsDown
                onClick={() => handleReaction("dislike")}
                className={`mt-1 hover:text-gray-500 duration-200 ${
                  activeReaction === "dislike" ? "text-blue-500" : ""
                }`}
              />
            </div>
            <div>

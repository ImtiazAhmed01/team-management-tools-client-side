...
const Task = ({ loggedInUserId }) => {
    ...
    useEffect(() => {
        axios.get("https://team-management-tools-server-side.onrender.com/tasks").then((response) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (taskData.id) {
                // UPDATE TASK
                await axios.put(`https://team-management-tools-server-side.onrender.com/tasks/${taskData.id}`, taskData);
                ...
                // CREATE TASK
                await axios.post("https://team-management-tools-server-side.onrender.com/tasks", taskData);
            ...
            // Refresh task list
            const response = await axios.get("https://team-management-tools-server-side.onrender.com/tasks");
            setTasks(response.data);
    ...
    const handleDelete = async (taskId) => {
        try {
            await axios.delete(`https://team-management-tools-server-side.onrender.com/tasks/${taskId}`);
            const response = await axios.get("https://team-management-tools-server-side.onrender.com/tasks");
            setTasks(response.data);
        } catch (error) {
...
const TaskCard = ({ task, loggedInUserId, onDelete, onEdit }) => {
        ...
        const checkIfAssigned = async () => {
            try {
                const { data } = await axios.get(
                    `https://team-management-tools-server-side.onrender.com/is-assigned/${task._id}/${user?.email}`
                );
    ...
    const handleAssignTask = async () => {
        if (!assigned) {
            ...
            try {
                const { data } = await axios.post("https://team-management-tools-server-side.onrender.com/assign-task", {
                    task,
                    userId: loggedInUserIds,
                    email: user?.email,
                });
    ...
    useEffect(() => {
        const fetchReaction = async () => {
            const { data } = await axios.get(
                `https://team-management-tools-server-side.onrender.com/reaction/${task._id}`
            );
            setReaction({
    ...
    const handleReaction = async (reactType) => {
        ...
        try {
            const { data } = await axios.post("https://team-management-tools-server-side.onrender.com/reactions", {
                cardId: task._id,
                reactions: reactType,
            });
            setReaction({
